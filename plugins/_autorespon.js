    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? `${namabot} tidak aktif` : banned ? 'kamu dibanned' : `ya?`,
                footer,
...
╠═〘 ${namabot} 〙 ═`.trim(), footer, 'Tng', '#viatng', 'Boost', '#viaboost', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`Aktif selama ${uptime} ⏲️| Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Hanya Grup' : 'Publik'} 👾| ${namabot} by ${namalu}`).catch(_ => _)
